...
<script>
	...
	export default {
		...
		props: {
			...
			topbar: Boolean, // top的偏移量是否加上状态栏高度 (当fixed为false时不生效; 使用场景:取消原生导航栏时,配置此项可自动加上状态栏高度的偏移量)
		...
		created() {
			...
			uni.getSystemInfo({
				success(res) {
					...
					if (res.statusBarHeight && vm.topbar) vm.windowTop += res.statusBarHeight; // 加上状态栏的高度
