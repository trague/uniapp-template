<template>
	<div class="ba-checker-item" :class="classNames" @click="select"><slot></slot></div>
</template>
...
export default {
	...
	computed: {
		parent() {
			...
			while (parent && parent.baName !== 'BaChecker') {
		...
		classNames() {
			...
			const names = []
			if (this.disabled) {
				names.push('ba-checker-item--disabled')
			}
			...
			if (selected) {
				names.push('ba-checker-item--active')
			}
